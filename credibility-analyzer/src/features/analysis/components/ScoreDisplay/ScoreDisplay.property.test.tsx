...
describe('Property 4: Score Color Mapping', () => {
  ...
  it('should display yellow and Moderate Credibility for scores 41-70', () => {
    fc.assert(
      fc.property(moderateScoreArb, (score) => {
        const category = getScoreCategory(score);
    ...
    fc.assert(
      fc.property(validScoreArb, (score) => {
        ...
        const { container, unmount } = render(
          <ScoreDisplay score={score} timestamp={timestamp} onShare={mockOnShare} onDownload={mockOnDownload} />
        );
  ...
  it('should apply correct color to credibility label', { timeout: 30000 }, () => {
    fc.assert(
      fc.property(validScoreArb, (score) => {
        ...
        const category = getScoreCategory(score);
        const credibilityLabel = within(container).getByTestId('credibility-label');
        const expectedRgb = hexToRgb(category.color);
        expect(credibilityLabel.style.color).toBe(expectedRgb);
        unmount();
      }),
      { numRuns: 100 }
    );
...
describe('Property 5: Timestamp Display', () => {
  ...
  it('should display a timestamp for any analysis', { timeout: 30000 }, () => {
    fc.assert(
      fc.property(validScoreArb, validDateArb, (score, timestamp) => {
        cleanup();
        const mockOnShare = vi.fn();
        const mockOnDownload = vi.fn();
        const { container, unmount } = render(
  ...
  it('should display timestamp containing correct date components', { timeout: 30000 }, () => {
    fc.assert(
      fc.property(validScoreArb, validDateArb, (score, timestamp) => {
        ...
        expect(displayedText).toContain(year);
        const day = timestamp.getDate().toString();
        expect(displayedText).toContain(day);
        unmount();
      }),
      { numRuns: 100 }
    );
